// ------------------------ КОММЕНТАРИИ И ПРАВКИ К ЗАДАНИЮ 1 START------------------------ 
// + имя PR не по формату
// + вместо прописывания и больших и маленьких букв лучше использовать флаг регистра
// + [_]{1} если есть что-то, что захардкожено - т.е. юзается 1 раз - то скобки и {1} не нужно - поправьте везде
// + [0-9] можно сократить - поправьте везде
// + {0,1} можно сократить - поправьте везде

// FIXED регулярное выражение:
/^[a-z]{3,10}_[a-z]{3,10}(-[\d]{4})?@[a-z\d]{1,10}[\.-]?[a-z\d]{1,10}\.com$/i.test('name_surname-1234@gmail.com')

// ------------------------ КОММЕНТАРИИ И ПРАВКИ К ЗАДАНИЮ 1 END ------------------------ 


// ------------------------ КОММЕНТАРИИ И ПРАВКИ К ЗАДАНИЮ 2 START------------------------ 

// + могу поставить плюс перед 8
// + могу поставить тире после 80-
// + тире не надо экранировать
// + {0,1}[0-9]{2} дублируется, можно сократить

// FIXED регулярное выражение:
/^((8-?0)|(\+?375-?))(17|25|29|33|44)-?[1-9]\d{2}(-?\d{2}){2}$/.test('+375-25-777-77-77')

// ------------------------ КОММЕНТАРИИ И ПРАВКИ К ЗАДАНИЮ 2 END ------------------------ 

// ------------------------ КОММЕНТАРИИ И ПРАВКИ К ЗАДАНИЮ 3 START------------------------ 

// + нет проверки на ситуацию, когда гласных 0

// FIXED регулярное выражение:

function vowelCount(str) {
    var result = str.match(/[AEIOUYaeiouy]/g);
    if (result == null) {
        return 0;
...
// ------------------------ КОММЕНТАРИИ И ПРАВКИ К ЗАДАНИЮ 3 END ------------------------ 
